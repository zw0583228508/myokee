    ...
    <div className="border-t border-white/[0.04] mt-3 pt-3">
      <div className="max-h-48 overflow-y-auto space-y-2 mb-3">
        {(data?.comments || []).map((c: any) => (
          <div key={c.id} className="flex gap-2 text-sm">
            <div className="w-6 h-6 rounded-full bg-gradient-to-br from-primary/50 to-accent/50 flex items-center justify-center flex-shrink-0 shadow-sm shadow-primary/10">
            ...
            <div>
              <span className="text-white/60 font-medium text-xs">{c.display_name}</span>
              <p className="text-white/40 text-xs">{c.content}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-1.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-primary/40 transition-colors"
          onKeyDown={e => {
      ...
      <div className="relative">
        <div className="flex items-center gap-3 mb-3">
          <Link href={`/profile/${perf.user_id}`}>
            <div className="cursor-pointer">
              {perf.picture ? (
                <img src={perf.picture} alt="" className="w-10 h-10 rounded-full object-cover ring-2 ring-white/[0.06]" />
              ) : (
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center shadow-lg shadow-primary/15">
          ...
          <div className="flex-1 min-w-0">
            <Link href={`/profile/${perf.user_id}`}>
              <span className="text-sm font-medium text-white/80 hover:text-primary cursor-pointer transition-colors">{perf.display_name}</span>
            </Link>
            <p className="text-xs text-white/20">{new Date(perf.created_at).toLocaleDateString()}</p>
          </div>
          <div className={`text-2xl font-bold ${scoreColor} drop-shadow-[0_0_8px_currentColor]`} style={{ filter: "brightness(1.1)" }}>{perf.score}</div>
        </div>

        <div className="flex items-center gap-2 mb-3">
          <Music className="w-4 h-4 text-primary/50" />
          <span className="text-sm text-white/60">{perf.song_name || "Unknown"}</span>
        </div>

        <div className="flex gap-4 text-sm text-white/30">
          <div className="flex gap-1">
            <span className="text-white/20">Pitch:</span>
            <span className="text-white/50">{perf.pitch_score}</span>
          </div>
          <div className="flex gap-1">
            <span className="text-white/20">Timing:</span>
            <span className="text-white/50">{perf.timing_score}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-4 pt-3 border-t border-white/[0.04]">
          <button
            ...
            className={`flex items-center gap-1.5 text-sm transition-all duration-300 ${perf.liked_by_me ? "text-red-400 drop-shadow-[0_0_6px_rgba(248,113,113,0.4)]" : "text-white/30 hover:text-red-400"}`}
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-white/30 hover:text-primary transition-colors"
          >
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-8" dir={isRtl ? "rtl" : "ltr"}>
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-white mb-1 font-display">{t.title}</h1>
        <p className="text-white/30 text-sm">{t.subtitle}</p>
      </div>

      <div className="flex gap-1 bg-white/[0.02] border border-white/[0.05] p-1 rounded-xl mb-6">
        {(["discover", "feed"] as const).map(key => (
          <button
            key={key}
            onClick={() => { setTab(key); setPage(0); }}
            className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all duration-300 ${
              tab === key ? "bg-primary/15 text-primary shadow-[0_0_20px_rgba(147,51,234,0.1)]" : "text-white/30 hover:text-white/60"
            }`}
          >
            {t[key]}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-white/30 text-lg">{tab === "feed" ? t.noFeed : t.noDiscover}</p>
          {tab === "feed" && <p className="text-white/20 text-sm mt-2">{t.noFeedDesc}</p>}
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {data?.hasMore && (
            <button
              ...
              className="w-full py-3 text-sm text-white/30 hover:text-white/60 flex items-center justify-center gap-2 transition-colors"
            >
