...
const content: Partial<Record<SupportedLang, Section[]>> = {
  he: [
    ...
    { title: "5. זכויותיך", body: "בהתאם לחוק הגנת הפרטיות הישראלי ו-GDPR (לאזרחי האיחוד האירופי), יש לך זכות:", bullets: ["לעיין במידע שנאסף עליך.", "לבקש מחיקת חשבונך ונתוניך.", "לבקש תיקון מידע שגוי."], contactHtml: "לפניות: support@myoukee.com" },
  ...
  ar: [
    ...
    { title: "5. حقوقك", body: "وفقًا لقانون حماية الخصوصية الإسرائيلي و-GDPR (لمواطني الاتحاد الأوروبي)، لديك الحق في:", bullets: ["مراجعة المعلومات المجمعة عنك.", "طلب حذف حسابك وبياناتك.", "طلب تصحيح المعلومات غير الدقيقة."], contactHtml: "للاستفسارات: support@myoukee.com" },
  ...
  return (
    <div className="min-h-screen bg-[var(--ds-bg-app)] relative" dir={t.dir}>
      <div className="absolute top-0 inset-x-0 h-[300px] -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="ds-orb ds-orb-violet absolute -top-32 left-1/3 w-[400px] h-[400px] opacity-50" />
      ...
      <div className="w-full max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-4 mb-10 ds-reveal">
          <div className="ds-icon-orb w-14 h-14 rounded-2xl">
            <Shield className="w-7 h-7 text-white" />
          </div>
          <h1 className="ds-page-title font-display font-bold text-white">{t.consent.privacyLink}</h1>
        </div>
        ...
        <div className="ds-card p-6 sm:p-8 space-y-7 text-white/65 leading-relaxed text-sm">
          {sections.map((s, i) => (
            <section key={i}>
              ...
              {s.body && <p>{s.body}</p>}
              {s.bullets && (
                <ul className="list-disc list-inside space-y-1.5 mt-3 marker:text-violet-400/60">
                  {s.bullets.map((b, j) => <li key={j} className="pl-1">{b}</li>)}
                </ul>
              )}
              {s.contactHtml && (
                <p className="mt-3">
                  {s.contactHtml.split("support@myoukee.com")[0]}
                  <a href="mailto:support@myoukee.com" className="text-violet-300 hover:text-violet-200 underline">support@myoukee.com</a>
                </p>
              )}
            </section>
          ))}
