...
const content: Partial<Record<SupportedLang, Section[]>> = {
  he: [
    ...
    { title: "4. קרדיטים ותשלומים", bullets: ["90 שניות ראשונות הן חינמיות בכל עיבוד.", "קרדיטים הם אמצעי תשלום ואינם ניתנים להחזר.", "מחירים עשויים להשתנות עם הודעה מוקדמת.", "עסקאות מעובדות דרך Stripe בצורה מאובטחת."] },
  ...
  en: [
    ...
    { title: "4. Credits and Payments", bullets: ["The first 90 seconds are free for every processing.", "Credits are a payment method and are non-refundable.", "Prices may change with prior notice.", "Transactions are processed securely through Stripe."] },
  ...
  ar: [
    ...
    { title: "4. الأرصدة والمدفوعات", bullets: ["أول 90 ثانية مجانية في كل معالجة.", "الأرصدة هي وسيلة دفع وغير قابلة للاسترداد.", "قد تتغير الأسعار مع إشعار مسبق.", "تتم معالجة المعاملات بأمان عبر Stripe."] },
  ...
  ru: [
    ...
    { title: "4. Кредиты и платежи", bullets: ["Первые 90 секунд бесплатны при каждой обработке.", "Кредиты являются средством оплаты и не подлежат возврату.", "Цены могут изменяться с предварительным уведомлением.", "Транзакции обрабатываются безопасно через Stripe."] },
  ...
  es: [
    ...
    { title: "4. Créditos y Pagos", bullets: ["Los primeros 90 segundos son gratuitos en cada procesamiento.", "Los créditos son un medio de pago y no son reembolsables.", "Los precios pueden cambiar con aviso previo.", "Las transacciones se procesan de forma segura a través de Stripe."] },
  ...
  fr: [
    ...
    { title: "4. Crédits et Paiements", bullets: ["Les 90 premières secondes sont gratuites pour chaque traitement.", "Les crédits sont un moyen de paiement et ne sont pas remboursables.", "Les prix peuvent changer avec un préavis.", "Les transactions sont traitées en toute sécurité via Stripe."] },
  ...
  de: [
    ...
    { title: "4. Guthaben und Zahlungen", bullets: ["Die ersten 90 Sekunden sind bei jeder Verarbeitung kostenlos.", "Guthaben ist ein Zahlungsmittel und nicht erstattungsfähig.", "Preise können sich mit vorheriger Ankündigung ändern.", "Transaktionen werden sicher über Stripe abgewickelt."] },
  ...
  return (
    <div className="min-h-screen bg-[var(--ds-bg-app)] relative" dir={t.dir}>
      <div className="absolute top-0 inset-x-0 h-[300px] -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="ds-orb ds-orb-violet absolute -top-32 left-1/3 w-[400px] h-[400px] opacity-50" style={{ animationDelay: "1s" }} />
      ...
      <div className="w-full max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-4 mb-10 ds-reveal">
          <div className="ds-icon-orb w-14 h-14 rounded-2xl" style={{ background: "linear-gradient(135deg,#06B6D4,#8B5CF6)", boxShadow: "0 0 32px rgba(6,182,212,.45)" }}>
            <FileText className="w-7 h-7 text-white" />
          </div>
          <h1 className="ds-page-title font-display font-bold text-white">{t.consent.termsLink}</h1>
        </div>
        ...
        <div className="ds-card p-6 sm:p-8 space-y-7 text-white/65 leading-relaxed text-sm">
          {sections.map((s, i) => (
            <section key={i}>
              ...
              {s.body && <p>{s.body}</p>}
              {s.bullets && (
                <ul className="list-disc list-inside space-y-1.5 mt-3 marker:text-cyan-400/60">
